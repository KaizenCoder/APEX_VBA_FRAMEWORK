...
# Configuration des chemins APEX Framework
ROOT = find_project_root()
LOGS_DIR = ROOT / "logs"
TOOLS_DIR = ROOT / "tools" / "python" / "cursor_logger"
CONFIG_DIR = ROOT / "config" / "cursor"
TEMPLATES_DIR = ROOT / "tools" / "templates" / "cursor"
HISTORY_DIR = ROOT / "logs" / "history" / "cursor_sessions"

# Fichiers de journalisation
JOURNAL_FILE = LOGS_DIR / "apex-cursor-journal.md"
SESSION_TEMPLATE = TEMPLATES_DIR / "session_template.md"
AUTOLOG_SCRIPT = TOOLS_DIR / "apex_cursor_autolog.py"
POWERSHELL_WRAPPER = TOOLS_DIR / "Log-ApexCursor.ps1"
CONFIG_FILE = CONFIG_DIR / "cursor_logger_config.json"
...
# -----------------------------------------------------------------------------
# Script: apex_cursor_autolog.py
# Description: Script de journalisation automatique pour APEX Framework
# Author: APEX Framework Team
# Date: 2024-04-11
# Version: 2.1
# -----------------------------------------------------------------------------
...
ROOT = find_project_root()
LOG_PATH = ROOT / "logs" / "apex-cursor-journal.md"
CONFIG_PATH = ROOT / "config" / "cursor" / "cursor_logger_config.json"
HISTORY_DIR = ROOT / "logs" / "history" / "cursor_sessions"
HISTORY_INDEX = HISTORY_DIR / "sessions_index.json"
...
def log_cursor_interaction(prompt, response, agent="Claude 3.7 Sonnet", note="+", session_id=None):
    """Enregistre une interaction avec Cursor/Claude dans le journal APEX"""
...
"""
    ...
    session_file = ROOT / "logs" / f"cursor_session_{session_id}.md"
...
def create_session_file(session_id=None, description=""):
    ...
    template_path = ROOT / "tools" / "templates" / "cursor" / "session_template.md"
    if template_path.exists():
        with open(template_path, "r", encoding="utf-8") as f:
            template = f.read()
    else:
        template = """# Session Cursor: {session_id}
Date: {date}
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: apex_cursor_autolog.py <command> [args...]")
        print("Commands:")
    ...
    if command == "log":
        if len(sys.argv) < 4:
            print("Usage: apex_cursor_autolog.py log '<prompt>' '<response>' [agent] [note] [session_id]")
            sys.exit(1)
        
        prompt = sys.argv[2]
        response = sys.argv[3]
        agent = sys.argv[4] if len(sys.argv) > 4 else "Claude 3.7 Sonnet" 
        note = sys.argv[5] if len(sys.argv) > 5 else "+"
        session_id = sys.argv[6] if len(sys.argv) > 6 else None
        
        log_cursor_interaction(prompt, response, agent, note, session_id)
        ...
        if len(sys.argv) < 3:
            print("Usage: apex_cursor_autolog.py archive-session <session_id> [description]")
            sys.exit(1)
...
SESSION_TEMPLATE_CONTENT = r'''# Session Cursor: {session_id}
Date: {date}
Description: {description}
...
def main():
    ...
    print("\n=== Installation du Logger APEX pour Cursor avec Historisation ===\n")
    ...
    if install_components():
        print("\n[✓] Logger APEX pour Cursor installé avec succès!")
        ...
        print("  - PowerShell: .\\Log-ApexCursor.ps1 -Command log -Prompt 'Question' -Response 'Réponse'")
        print("  - Python:     python apex_cursor_autolog.py log 'Question' 'Réponse'")
        
        print("\nPour créer une nouvelle session:")
        print("  - PowerShell: .\\Log-ApexCursor.ps1 -Command create-session -Description 'Description de la session'")
        print("  - Python:     python apex_cursor_autolog.py create-session 'Description de la session'")
        
        print("\nPour archiver une session terminée:")
        print("  - PowerShell: .\\Log-ApexCursor.ps1 -Command archive-session -SessionId '20240411-1530' -Description 'Résumé de la session'")
        print("  - Python:     python apex_cursor_autolog.py archive-session '20240411-1530' 'Résumé de la session'")
        
        print("\nPour lister les sessions:")
        print("  - PowerShell: .\\Log-ApexCursor.ps1 -Command list-sessions [-ShowAll]")
        print("  - Python:     python apex_cursor_autolog.py list-sessions [--all]")
